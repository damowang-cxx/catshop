...
export default function LoginPage({ params }: { params: { locale: Locale } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(err.message || t(locale, "common", "loginFailed"));
    }
  };
      ...
      <div className="max-w-md w-full space-y-8">
        <div className="text-center">
          <h2 className="mt-6 text-4xl font-extrabold text-rose-600 animate-fadeIn">
            🧸 {t(locale, "common", "loginTitle")}
          </h2>
          <p className="mt-2 text-sm text-stone-600">
            {t(locale, "common", "loginSubtitle")}
          </p>
        </div>
        <form className="mt-8 space-y-6 bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg p-8 border border-pink-200" onSubmit={handleSubmit}>
          ...
          <div>
            ...
            >
              {loading ? t(locale, "common", "loading") : t(locale, "common", "login")}
            </button>
          </div>

          <div className="text-sm text-center text-stone-600">
            <p>
              {t(locale, "common", "noAccount")}{" "}
              <Link
